...
void screenshot(char *pcxname) {
	...
	D_EnableBackBufferAccess ();	// enable direct drawing of console to back
									//  buffer
	WritePCXfile (pcxname, vid.buffer, vid.width, vid.height, vid.rowbytes, current_pal, false);
...
void SCR_ScreenShot_f (void) { 
	...
	if (Cmd_Argc() == 2) {
		...
		for (i = 0; i < 999; i++) {
			Q_snprintfz(name, sizeof(name), "ezquake%03i.pcx", i);
		...
		Com_Printf("Usage: screenshot [filename]");
		return;
	}
	filename =  (*name == '/') ? name + 1 : name;
	screenshot(va("%s/%s", cls.gamedirfile, filename));
} 
