			...
			if (!allowremotecmd && (!*localid.string || strcmp(localid.string, s))) 
			{
				...
				else 
				{
					...
					Com_Printf ("Invalid localid on command packet received from local host. "
						"\n|%s| != |%s|\n"
						"You may need to reload your server browser and FortressOne.\n",
						s, localid.string);
					Com_Printf ("===========================\n");
					Cvar_Set(&localid, "");
				}
...
void CL_InitLocal (void) 
{
	...
	snprintf(st, sizeof(st), "FortressOne %i", REVISION);
...
void CL_UpdateCaption(qbool force)
{
	...
	if (!cl_window_caption.value)
	{
		if (!cls.demoplayback && (cls.state == ca_active))
			snprintf(str, sizeof(str), "FortressOne: %s", cls.servername);
		else
			snprintf(str, sizeof(str), "FortressOne");
	}
	else
	{
