...
void Help_Issues_Commands(void)
{
	...
	for (iterator = json_object_iter(command_root); iterator; iterator = json_object_iter_next(command_root, iterator)) {
		const char* name = json_object_iter_key(iterator);
		json_t* definition = json_object_iter_value(iterator);
	...
	json_object_foreach(varsObj, name, variable) {
		...
		// old variables might still be in documentation, allows "/describe <var>" to explain why it is removed
		{
			...
			for (cvar = cvar_vars; cvar && !in_client; cvar = cvar->next) {
				in_client = !stricmp(cvar->name, name);
			}
			if (!in_client) {
				continue;
			}
