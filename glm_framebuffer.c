...
static void VID_FramebufferFlip(void)
{
	...
	if (flip3d || flip2d) {
		...
		if (GLM_CompilePostProcessProgram()) {
			...
			if (flip2d && flip3d) {
				renderer.TextureUnitBind(0, GL_FramebufferTextureReference(framebuffer3d, 0));
				renderer.TextureUnitBind(1, GL_FramebufferTextureReference(framebuffer2d, 0));
			}
			else if (flip3d) {
				renderer.TextureUnitBind(0, GL_FramebufferTextureReference(framebuffer3d, 0));
			}
			else if (flip2d) {
				renderer.TextureUnitBind(0, GL_FramebufferTextureReference(framebuffer2d, 0));
			}
