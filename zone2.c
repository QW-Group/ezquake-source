...
cvar_t developer_memory = {"developer_memory", "0"};
cvar_t developer_memorydebug = {"developer_memorydebug", "0"};
...
void *_Mem_Alloc(mempool_t *pool, size_t size, const char *filename, int fileline)
{
	...
	if (developer.integer && developer_memory.integer)
		Com_Printf("Mem_Alloc: pool %s, file %s:%i, size %i bytes\n", pool->name, filename, fileline, (int)size);
...
static void _Mem_FreeBlock(memheader_t *mem, const char *filename, int fileline)
{
	...
	pool = mem->pool;
	if (developer.integer && developer_memory.integer)
		Com_Printf("Mem_Free: pool %s, alloc %s:%i, free %s:%i, size %i bytes\n", pool->name, mem->filename, mem->fileline, filename, fileline, (int)(mem->size));
...
void Mem_PrintStats(void)
{
	...
	Com_Printf("%lu memory pools, totalling %lu bytes (%.3fMB)\n", (unsigned long)count, (unsigned long)size, size / 1048576.0);
	Com_Printf("total allocated size: %lu bytes (%.3fMB)\n", (unsigned long)realsize, realsize / 1048576.0);
	for (pool = poolchain;pool;pool = pool->next)
	{
		if ((pool->flags & POOLFLAG_TEMP) && pool->chain)
		{
			Com_Printf("Memory pool %p has sprung a leak totalling %lu bytes (%.3fMB)!  Listing contents...\n", pool, (unsigned long)pool->totalsize, pool->totalsize / 1048576.0);
			for (mem = pool->chain;mem;mem = mem->next)
				Com_Printf("%10lu bytes allocated at %s:%i\n", (unsigned long)mem->size, mem->filename, mem->fileline);
		}
	}
}
...
void Mem_PrintList(size_t minallocationsize)
{
	...
	for (pool = poolchain;pool;pool = pool->next)
	{
		Com_Printf("%10luk (%10luk actual) %s (%+li byte change) %s\n", (unsigned long) ((pool->totalsize + 1023) / 1024), (unsigned long)((pool->realsize + 1023) / 1024), pool->name, (long)pool->totalsize - pool->lastchecksize, (pool->flags & POOLFLAG_TEMP) ? "TEMP" : "");
...
void Memory2_Init_Commands (void)
{
	Cmd_AddCommand ("memstats", MemStats_f);
	Cmd_AddCommand ("memlist", MemList_f);

	Cvar_Register (&developer_memory);
	Cvar_Register (&developer_memorydebug);
}
...
char *Z_Strdup (const char *src)
{
	...
	alloclen = strlen (src) + 1;
	data = Z_Malloc (alloclen);
	memcpy (data, src, alloclen);
