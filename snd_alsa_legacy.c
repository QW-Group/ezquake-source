...
#include "snd_alsa_legacy_funcs.h"
#undef ALSA_FUNC
...
// Main functions
int SNDDMA_GetDMAPos_ALSA_Legacy (void)
{
...
void SNDDMA_Shutdown_ALSA_Legacy (void)
{
	alsa_snd_pcm_close (pcm);
}

void SNDDMA_Submit_ALSA_Legacy (unsigned int count)
{
	int state;
//	int count = paintedtime - soundtime; //TEST not neccessary i guess since its sent in from caller..
	const snd_pcm_channel_area_t *areas;
...
#undef ALSA_FUNC
	...
	pcmname = Cvar_String("s_alsa_device");
	...
	SNDDMA_GetDMAPos_ALSA_Legacy(); // sets shm->buffer
	sd->GetDMAPos = SNDDMA_GetDMAPos_ALSA_Legacy;
	sd->GetAvail = NULL;
	sd->Submit = SNDDMA_Submit_ALSA_Legacy;
	sd->Shutdown = SNDDMA_Shutdown_ALSA_Legacy;
	sd->name = "snd_alsa (legacy)";
