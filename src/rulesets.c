...
qbool RuleSets_DisallowExternalTexture(struct model_s *mod)
{
	switch (mod->modhint) {
		case MOD_EYES:
			return true;
		case MOD_BACKPACK:
			return rulesetDef.ruleset == rs_smackdown || rulesetDef.ruleset == rs_qcon || rulesetDef.ruleset == rs_2024;
		default:
			return false;
	}
}
...
qbool RuleSets_DisallowModelOutline(struct model_s *mod)
{
	...
	switch (mod->modhint) {
		...
		case MOD_THUNDERBOLT:
			return true;
		case MOD_BACKPACK:
			return !cls.demoplayback && (rulesetDef.ruleset == rs_qcon || rulesetDef.ruleset == rs_smackdown || rulesetDef.ruleset == rs_2024);
		default:
...
static void Rulesets_2024(qbool enable)
{
	extern cvar_t cl_independentPhysics, cl_c2spps;
	extern cvar_t cl_hud;
	extern cvar_t cl_rollalpha;
...
static void Rulesets_OnChange_ruleset(cvar_t *var, char *value, qbool *cancel)
{
	...
	if (strncasecmp(value, "smackdown", sizeof("smackdown")) &&
			...
			strncasecmp(value, "2024", sizeof("2024")) &&
	...
	} else if (!strncasecmp(value, "2024", sizeof("2024"))) {
		Rulesets_2024(true);
		Com_Printf_State(PRINT_OK, "Ruleset 2024 initialized\n");
