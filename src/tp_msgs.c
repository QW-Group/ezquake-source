...
LOCAL char* TP_MSG_Build_Inlay_String(void)
{
    ...
    // HACK: We return a pointer into this static string because we expect it to be used immediately.
    static char inlay_buffer[256];
    snprintf(inlay_buffer, sizeof(inlay_buffer), "#inlay# %s,%s,%s,%s,%s,%s", armor, health, weapon, powerups, location, message);
    return inlay_buffer;
}
...
{
    // FIXME: Send an invisible inlay message compatible with older clients.
    // We can send an invisible inlay message with:
    //   say_team <inlay>\r<nick> <msg>
    // Because the \r ("$\") carriage return restarts the display of the message.
	TP_Msg_Report_Inlay_f();
...
#define TP_MSG_GENERIC(type) \
	...
	TP_Send_TeamSay("%s" type " $[{%%l}$]", (HAVE_POWERUP() ? "$tp_powerups " : ""));
