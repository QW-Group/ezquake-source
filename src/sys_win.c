...
{
	// SDL's argv is sized exactly to argc+1 (only the NULL terminator is spare),
	// so copy into a local array before appending our own parameters.
	static char *args[64];
	int memsize, i;
	...
	if (!WinCheckQWURL() && argc + 2 < (int)(sizeof(args) / sizeof(args[0])))
	{
		// User doesn't want to be bothered again.
		for (i = 0; i < argc; i++) {
			args[i] = argv[i];
		}
		args[argc++] = "+cl_verify_qwprotocol";
		args[argc++] = "0";
		argv = args;
	}
