...
cvar_t			scr_mvdinfo		= {"scr_mvdinfo", "0"};
cvar_t			scr_mvdinfo_setup		= {"scr_mvdinfo_setup", "%n %f %l %a %h %w %x"};
cvar_t			scr_mvdinfo_x	= {"scr_mvdinfo_x", "0"};
cvar_t			scr_mvdinfo_y	= {"scr_mvdinfo_y", "0"};
...
int MVD_BestWeapon (int i) {
	...
	player_info_t *mvd_info;
	mvd_info = cl.players;
	for (s = t; *s; s++) {
		for (x = 0 ; x < strlen(*s) ; x++) {
			switch ((*s)[x]) {
				case '1': if (mvd_info[i].stats[STAT_ITEMS] & IT_AXE) return IT_AXE; break;
				case '2': if (mvd_info[i].stats[STAT_ITEMS] & IT_SHOTGUN) return IT_SHOTGUN; break;
				case '3': if (mvd_info[i].stats[STAT_ITEMS] & IT_SUPER_SHOTGUN) return IT_SUPER_SHOTGUN; break;
				case '4': if (mvd_info[i].stats[STAT_ITEMS] & IT_NAILGUN) return IT_NAILGUN; break;
				case '5': if (mvd_info[i].stats[STAT_ITEMS] & IT_SUPER_NAILGUN) return IT_SUPER_NAILGUN; break;
				case '6': if (mvd_info[i].stats[STAT_ITEMS] & IT_GRENADE_LAUNCHER) return IT_GRENADE_LAUNCHER; break;
				case '7': if (mvd_info[i].stats[STAT_ITEMS] & IT_ROCKET_LAUNCHER) return IT_ROCKET_LAUNCHER; break;
				case '8': if (mvd_info[i].stats[STAT_ITEMS] & IT_LIGHTNING) return IT_LIGHTNING; break;
			}
		}
	}
	return 0;
...
char *MVD_BestAmmo (int i) {
	...
	switch (MVD_BestWeapon(i)) {
		case IT_SHOTGUN: case IT_SUPER_SHOTGUN:
			Q_snprintfz(mvd_info_best_ammo, sizeof(mvd_info_best_ammo), "%i", mvd_info[i].stats[STAT_SHELLS]);
			return mvd_info_best_ammo;
				
		case IT_NAILGUN: case IT_SUPER_NAILGUN:
			Q_snprintfz(mvd_info_best_ammo, sizeof(mvd_info_best_ammo), "%i", mvd_info[i].stats[STAT_NAILS]);
			return mvd_info_best_ammo;
				
		case IT_GRENADE_LAUNCHER: case IT_ROCKET_LAUNCHER:
			Q_snprintfz(mvd_info_best_ammo, sizeof(mvd_info_best_ammo), "%i", mvd_info[i].stats[STAT_ROCKETS]);
			return mvd_info_best_ammo;
			
		case IT_LIGHTNING:
			Q_snprintfz(mvd_info_best_ammo, sizeof(mvd_info_best_ammo), "%i", mvd_info[i].stats[STAT_CELLS]);
			return mvd_info_best_ammo;
...
void SCR_MVDUsage_f (void) {
	Com_Printf("scr_mvdinfo_setup usage:\n");
	Com_Printf("%%a - Armor\n");
	Com_Printf("%%f - Frags\n");
	Com_Printf("%%h - Health\n");
...
void SCR_MVDInfo (void){
	...
	if (!scr_mvdinfo.value)
		return;
	
	if (!cls.mvdplayback)
		return;
	
	x = ELEMENT_X_COORD(scr_mvdinfo);
	y = ELEMENT_Y_COORD(scr_mvdinfo);
	//Q_snprintfz(str, sizeof(str),"%10s%6s%6s%7s%6s%10s","Name","Ping","Frags","Health","Armor","Location");
	//Draw_String (x, y, str);
	for ( i=0 ; i<MAX_CLIENTS ; i++ ){
	if (!mvd_info[i].name[0])
		continue;
	if (mvd_info[i].spectator)
		continue;
	...
	tmpstr=scr_mvdinfo_setup.string;
	...
	while (strstr(tmpstr,"%")){
		...
		switch ( info_type[0] ){
			case 'a' :
				...
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%s %3i",mvd_info_final_string_tmp,mvd_info[i].stats[STAT_ARMOR]);
				}else{
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%3i",mvd_info[i].stats[STAT_ARMOR]);
				}
				break;
			case 'f' :
				if(strlen(mvd_info_final_string)){
				Q_strncpyz(mvd_info_final_string_tmp,mvd_info_final_string,sizeof(mvd_info_final_string));
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%s %3i",mvd_info_final_string_tmp,mvd_info[i].frags);
				}else{
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%3i",mvd_info[i].frags);
				}
				break;
			case 'h' :
				if(strlen(mvd_info_final_string)){
				Q_strncpyz(mvd_info_final_string_tmp,mvd_info_final_string,sizeof(mvd_info_final_string));
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%s %3i",mvd_info_final_string_tmp,mvd_info[i].stats[STAT_HEALTH]);
				}else{
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%3i",mvd_info[i].stats[STAT_HEALTH]);
				}
				break;
			case 'l' :
				if(strlen(mvd_info_final_string)){
			...
			case 'n' :
				if(strlen(mvd_info_final_string)){
				Q_strncpyz(mvd_info_final_string_tmp,mvd_info_final_string,sizeof(mvd_info_final_string));
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%s %-10s",mvd_info_final_string_tmp,mvd_info[i].name);
				}else{
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%-10s",mvd_info[i].name);
				}
				break;
			case 'P' :
				if(strlen(mvd_info_final_string)){
				Q_strncpyz(mvd_info_final_string_tmp,mvd_info_final_string,sizeof(mvd_info_final_string));
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%s %3i",mvd_info_final_string_tmp,mvd_info[i].ping);
				}else{
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%3i",mvd_info[i].ping);
				}
				break;
			
			case 'p' :
					if (mvd_info[i].stats[STAT_ITEMS] & IT_QUAD)
						Q_strncpyz(mvd_info_powerups, tp_name_quad.string, sizeof(mvd_info_powerups));
				
					if (mvd_info[i].stats[STAT_ITEMS] & IT_INVULNERABILITY) {
					...
					if (mvd_info[i].stats[STAT_ITEMS] & IT_INVISIBILITY) {
						if (mvd_info_powerups[0])
						Q_strncatz(mvd_info_powerups, tp_name_separator.string);
			...
			case 'w' :
				if(strlen(mvd_info_final_string)){
				Q_strncpyz(mvd_info_final_string_tmp,mvd_info_final_string,sizeof(mvd_info_final_string));
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%s %3s:%-3i",mvd_info_final_string_tmp,Weapon_NumToString(mvd_info[i].stats[STAT_ACTIVEWEAPON]),mvd_info[i].stats[STAT_AMMO]);
				}else{
				Q_snprintfz(mvd_info_final_string,sizeof(mvd_info_final_string),"%3s:%-3i",Weapon_NumToString(mvd_info[i].stats[STAT_ACTIVEWEAPON]),mvd_info[i].stats[STAT_AMMO]);
				}
				break;
...
void SCR_Init (void) {
	...
	Cvar_Register (&scr_mvdinfo);
	Cvar_Register (&scr_mvdinfo_setup);
	Cvar_Register (&scr_mvdinfo_x);
	Cvar_Register (&scr_mvdinfo_y);

	Cmd_AddCommand ("screenshot", SCR_ScreenShot_f);
	Cmd_AddCommand ("sizeup", SCR_SizeUp_f);
	Cmd_AddCommand ("sizedown", SCR_SizeDown_f);
	Cmd_AddCommand ("scr_mvdinfo_setup_usage",SCR_MVDUsage_f);
